...
internal extension SKPhotoBrowser {
    ...
    @objc func longPressGestureRecognized(_ sender: UILongPressGestureRecognizer) {
        actionButtonPressed(ignoreAndShare: true)
    }
...
private extension SKPhotoBrowser {
    ...
    func configureGestureControl() {
        longPressGesture = UILongPressGestureRecognizer(target: self, action: #selector(SKPhotoBrowser.longPressGestureRecognized(_:)))
